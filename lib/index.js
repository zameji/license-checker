        ...
        if (options.excludePackages) {
            ...
            Object.keys(filtered).map(function(key) {
                ...
                excludedPackages.forEach(function(excludedPackage) {
                    // Version is present in excluded package name
                    if (excludedPackage.match(/.@/)) { // Don't match scoped packages
                        // Compare entire string including version
                        if (excludedPackage === key) {
                            isExcluded = true;
                        }
                    } else {
                        // Compare current package name with excluded package name
                        if (key.match(/^(.+)@/)[1] === excludedPackage) {
                            isExcluded = true;
                        }
                    }
                });
